<!--
  This file mirrors sdk/LLM.md, hoisted to the repo root so AI agents pick it up
  automatically (most tools ingest root-level Markdown by default, but do not recurse
  into vendored package directories).

  The canonical source is sdk/LLM.md — edit there first, then sync here. Link targets
  under “Monorepo template” use ./ paths here (repo root) vs ../ in sdk/LLM.md;
  prose should stay in sync.
-->
# `@useroot/sandbox-sdk` — AI Agent Cheat-Sheet
...
Canonical shapes live in [`sdk/src/types.ts`](./sdk/src/types.ts). Summarized for LLM prompts:
