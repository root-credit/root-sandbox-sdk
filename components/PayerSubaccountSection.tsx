...
export function PayerSubaccountSection({
  ...
  return (
    <Card className="mb-6 rounded-lg border border-border">
      <CardHeader>
        <CardTitle className="text-lg font-semibold tracking-tight">{branding.walletName}</CardTitle>
        <CardDescription>
          Your account balance for sending money abroad and receiving withdrawals. Powered by Root.
        </CardDescription>
      </CardHeader>
      <CardContent className="flex flex-col gap-6">
        <div className="flex flex-col gap-4 rounded-lg border border-border bg-card p-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-1">
            <p className="text-sm font-medium text-foreground">Enable {branding.walletName}</p>
            <p className="text-xs text-muted-foreground">
              Activate your account balance to fund and send money.
            </p>
          </div>
          <ToggleSwitch
        ...
        {subaccountEnabled ? (
          <div className="flex flex-col gap-4 rounded-lg border border-border bg-card p-4">
            <div>
              <p className="text-sm font-medium text-foreground">Add money (ACH deposit)</p>
              <p className="text-xs text-muted-foreground mt-1">
                Pull funds from your {branding.funderLabel.toLowerCase()} into your {branding.walletName}.
              </p>
            </div>
              ...
              <form onSubmit={handleSubmit(onPayinSubmit)} className="flex flex-col gap-4">
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    >
                      <option value="standard_ach">Standard ACH</option>
                      <option value="same_day_ach">Same day ACH</option>
                    </select>
                    {errors.rail ? (
