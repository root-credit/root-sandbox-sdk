...
- The **`test-app/` Express server** in this repo is only a **local developer harness** (quick buttons without scaffolding Next.js). It is **not** the production pattern for Vercel. Ignore it when imagining v0 → Vercel deploys.
...
```ts
import { statusLabel, isTerminal, isSuccess } from '@useroot/sandbox-sdk'
...
```ts
import { RootApiError } from '@useroot/sandbox-sdk'
...
- ❌ Import `root` (or `@useroot/sandbox-sdk`) into a `'use client'` component.
