...
export default function ActivityPage() {
  ...
  const successfulTransactions = transactions.filter((t) => isSuccessfulPayoutStatus(t.status)).length;
    ...
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <main className="flex-1 mx-auto max-w-7xl w-full px-6 lg:px-10 py-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
          <StatCard label={`Total ${branding.payoutVerb.toLowerCase()}`} value={formatMoney(totalPaidCents)} />
          <StatCard label={`Successful ${branding.payoutNounPlural.toLowerCase()}`} value={String(successfulTransactions)} />
          <StatCard label="Total events" value={String(transactions.length)} />
        </div>

        <div className="rounded-2xl border border-border bg-card overflow-hidden">
          {isLoading ? (
            <div className="p-12 text-center text-sm text-muted-foreground font-semibold">Loading activity…</div>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead className="font-bold uppercase tracking-widest text-[10px]">{branding.payeeSingular}</TableHead>
                  <TableHead className="font-bold uppercase tracking-widest text-[10px]">Email</TableHead>
                  <TableHead className="text-right font-bold uppercase tracking-widest text-[10px]">Amount</TableHead>
              ...
              <TableBody>
                {transactions.map((transaction) => (
                  <TableRow key={transaction.id}>
                    ...
                    <TableCell className="text-muted-foreground font-mono text-xs">{transaction.payeeEmail}</TableCell>
...
function StatusBadge({ status }: { status: string }) {
  ...
  const { label, variant } = map[key] ?? { label: status.replace(/_/g, ' '), variant: 'secondary' as const };
